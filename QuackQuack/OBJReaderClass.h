...
private:
	...
	std::vector<TriangleFacePosition> triangles;
	...
	std::vector<TriangleFacePosition> triangulate(const std::vector<objVertexData> face);

	std::vector<Vertex> convertToVertexObjects();


	std::vector<Vertex> computePositionVertices();
	std::vector<uint32_t> computePositionIndices();
